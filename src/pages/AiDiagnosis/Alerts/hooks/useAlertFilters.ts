...
const INITIAL: AlertFilterState = { type: 'all', severity: 'all', handled: 'all', sort: 'recent' };

/**
 * 알림 목록 필터. 발전소 선택과 조회 기간은 전역 값을 따르고,
 * 유형·심각도·조치여부·정렬만 화면 안에서 관리한다.
 */
export function useAlertFilters(options: { forcePending?: boolean } = {}) {
  ...
  const isDirty = filters.type !== 'all' || filters.severity !== 'all' || filters.handled !== 'all';
