 ...
 * 견주게 한다 (SFR-007-03). 같은 값을 숫자로 확인하고 싶은 사람을 위해 표로도 바꿀 수 있다.
 */
  ...
  return (
    <Reveal delay={0.1}>
      <Card
        ...
        description={`${describeDetail(period, date)} · 최고는 ${detail[peakIndex]?.label ?? '—'}, 합계 ${formatNumber(total / unit.divider, 2)}${unit.unit}입니다. 점선 막대는 ${COMPARE_LABEL[period]}이라 같은 눈금에서 견줄 수 있습니다. 표로 바꾸면 같은 값을 숫자로 봅니다.`}
        action={(
          <SegmentedControl
            label="보기 방식"
            size="sm"
