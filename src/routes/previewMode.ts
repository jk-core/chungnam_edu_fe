...
const OPEN_PREFIXES = [
  ...
  PATH.KIOSK,
  /*
    관리자 콘솔은 열지 않는다 (2026-09-16 지시).

    시연에서 보여 줄 것은 상황판과 교육용 화면이다. 콘솔은 계정·발전소·설비를 실제로 고치는
    자리라 로그인 없이 열어 두면 시연 자리에서 누른 것이 그대로 저장된다. 주소를 짚어 들어와도
    고르개로 모인다 — 목록에서 줄만 빼면 링크가 없어질 뿐 주소는 그대로 열린다.
  */
] as const;
