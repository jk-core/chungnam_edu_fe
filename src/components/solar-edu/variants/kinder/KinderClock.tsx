...
import { KinderFlow, KinderGiftArt, KinderGoodArt, KinderSchool, KinderSun, KinderWord } from '../../kinder/KinderArt';
import styles from './KinderClock.module.scss';
...
/**
 * 유치원 판 · 시안 B — 해님이 어디 있나요 (SFR-005-01/02/03/04/05/06/07/08).
 *
 * 현행 시안은 이야기를 걸음으로 나눠 차례로 보여 준다. 잘 읽히지만 기다려야 한다 —
 * 유치원 아이가 복도에서 화면 앞에 머무는 시간은 몇 초이고, 그 몇 초에 걸린 걸음 하나만 보고 지나간다.
 *
 ...
 */
export function KinderClock({ stats, content, nowHour }: KinderClockProps) {
        ...
        <section className={styles.clock}>
          <div className={styles.headRow}>
            ...
            {/*
              ...
              수 있으니 유치원에 맞는 방식이라 보았는데, 실제로 걸어 보니 **무엇을 세고 있는 건지**가
          ...
          <div className={styles.clock__canvas}>
            <svg viewBox={`0 0 ${VIEW.w} ${VIEW.h}`} fill="none" role="img" aria-label={`${word}. ${line}`} preserveAspectRatio="xMidYMid meet">
              ...
              */}
              <KinderSchool {...SCHOOL} lit={stats.loadRatio > 0 || !isDay} />
          ...
          <KinderWord word={word} line={line} />
        </section>
