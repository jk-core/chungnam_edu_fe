...
import { Box, Building, House, RoofPanel, Sun, Window } from '../scene-art/SceneParts';
import styles from './KinderArt.module.scss';
import type { CSSProperties } from 'react';

/*
  유치원 판 · 입체 그림 (SFR-005-04/06).

  평면 그림(`KinderArt`)과 **같은 물건, 같은 자리, 같은 크기**를 그린다. 다른 것은 명암뿐이다 —
  ...
  같은 `SceneDefs` 를 본다. 유치원만 다른 광원을 쓰면 같은 학교의 화면 넷이 서로 다른 곳처럼 보인다.
 ...
 * 입체는 생겼는데 유치원 그림이 아니게 됐다 — 멀리서 보면 형태가 물러 흐물거렸다. 부품이 그은 선
    ...
    <Building x={x} y={y} w={w} h={h} depth={w * 0.075}>
      ...
      */}
      ...
      {/*
        ...
        유치원 화면에서는 형태가 잡히지 않는다. 그 위에 한 겹 덧그어 윤곽만 굵게 만든다.
      */}
