...
import { ReliefFlow, ReliefSchool, ReliefSun } from './KinderReliefArt';
import styles from './KinderJourney.module.scss';

/**
 * 햇빛이 전기가 되어 교실에 닿기까지 — 입체 한 장 (SFR-005-01/02/06/07).
 *
 * 평면 쪽(`KinderJourney`)과 **같은 뷰박스에 같은 물건을 같은 자리로** 세운다. 해도 학교도 길도
 * 좌표가 같고, 달라진 것은 그 물건들이 면을 갈라 두께를 얻었다는 것뿐이다 — 두 시안을 나란히
 * 놓고 골라야 하므로 차이가 「입체감 하나」로 좁혀져 있어야 한다.
 *
 * 걸음을 나누지 않는다. 이 그림을 쓰는 시안은 처음부터 다 켜 두는 쪽 하나뿐이라, 평면 쪽이 갖고 있는
 * 걸음별 등장 장치를 여기까지 끌고 오면 쓰이지 않는 길이 생긴다.
 */
export function KinderReliefJourney({ nowHour, label }: { nowHour: number; label: string }) {
