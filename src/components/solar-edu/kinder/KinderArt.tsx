...
import styles from './KinderArt.module.scss';
import type { CSSProperties, ReactNode } from 'react';

/*
  유치원 판이 나눠 쓰는 그림 부품 (SFR-005-04/06/07).

  초등 이상이 쓰는 `scene-art` 부품을 그대로 줄여 쓰지 않는다. 그쪽은 **사실에 가깝게** 그린 것이라
  면을 여러 겹 갈라 두께를 만드는데, 멀리서 보는 유치원 화면에서는 그 겹이 뭉개져 회색 덩어리가 된다.
  ...
  색은 기존 토큰만 쓴다. 유치원이라고 색을 새로 들이면 같은 학교의 화면 넷이 서로 다른 곳처럼 보인다.
*/

/** 굵은 테두리 한 겹 — 유치원 판의 모든 도형이 같은 굵기를 쓴다 */
const STROKE = 5;
 ...
 * 그리고 가운데 현관. 여기에 유치원 아이가 학교를 알아보는 표시 하나를 더 얹었다 — 옥상의 시계다.
 *
    ...
    <g transform={`translate(${x} ${y})`}>
      ...
      */}
      ...
      {/*
        ...
        아이도 시계는 알아본다 — 유치원 교실에도 하나씩 걸려 있다.
 ...
 * 유치원에서 보던 아이가 초등에 올라가 같은 값을 만났을 때 같은 것을 말하고 있다는 걸 알아본다.
 */
export function KinderGiftArt({ id }: { id: 'tree' | 'aircon' | 'house' }) {
  if (id === 'tree') {
    return (
 ...
 */
export function KinderGoodArt({ id }: { id: 'free' | 'clean' | 'quiet' | 'roof' }) {
  if (id === 'free') {
    return (
      <svg viewBox={`0 0 ${BOX} ${BOX}`} fill="none" role="presentation">
        <SceneDefs />
 ...
 * 유치원 화면이 아니게 됐다. 큰 한 마디와 그 아래 짧은 한 줄, 둘로 나누면 멀리서는 한 마디만
 * 읽히고 가까이 선 아이에게는 선생님이 아랫줄을 읽어 줄 수 있다.
 */
export function KinderWord({ word, line }: { word: ReactNode; line?: string }) {
  return (
