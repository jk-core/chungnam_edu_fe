...
export function useStatisticsScopeRoute() {
  const { plantId, inverterId } = useParams<{ plantId: string; inverterId: string }>();
  ...
  const urlNode = getNode(inverterId ?? plantId ?? fallbackPlantId);
  ...
  useEffect(() => {
    ...
    navigate(buildPath.energyStatistics(fallbackPlantId), { replace: true });
  }, [plantId, fallbackPlantId, navigate]);
  ...
  useEffect(() => usePlantStore.subscribe((state, prev) => {
    ...
    // 인버터보다 깊은 자리는 그 인버터로 접어 넣는다 — 접속반·스트링은 AI진단 몫이다.
    if (inverter && inverter.id !== state.selectedNodeId) {
      selectNode(inverter.id);

      return;
    }

    const next = buildPath.energyStatistics(path.find((item) => item.kind === 'plant')?.id, inverter?.id);
