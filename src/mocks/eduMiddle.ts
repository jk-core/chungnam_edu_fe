...
export const MIDDLE_CONTENT: MiddleContent = {
  ...
  headline: {
    mainLabel: '실시간 출력',
    mainNote: (stats) =>
      `설비용량 ${formatNumber(stats.capacityKw)}kW 와 견주면 지금 이만큼 내고 있다`,
    ...
    copy: {
      ...
      insolation: {
        note: () => '발전량을 설비용량으로 나눈 값이다. 크기가 다른 설비를 견줄 때 쓴다',
      },
      co2: {
    ...
    stages: [
      {
        ...
        term: '햇빛이 지붕에 닿는다',
        body:
          '태양이 높이 뜰수록 햇빛이 판에 가깝게 수직으로 내리쬔다. 같은 양의 빛이 좁은 면적에 모이므로 '
          + '판 1m² 가 받는 에너지가 커지고, 그만큼 발전량도 늘어난다. 아침과 저녁에는 빛이 비스듬히 들어오는 '
          + '데다 지나야 할 공기층까지 두꺼워, 판에 닿을 때는 이미 에너지를 잃은 뒤다.',
      },
      {
        id: 'cell',
        step: 2,
      ...
      },
      {
        ...
        term: '인버터가 교류로 바꾼다',
        body:
          '판이 만든 전기는 한 방향으로만 흐르는 직류다. 그런데 교실 콘센트에 오는 전기는 방향이 1초에 '
      ...
      },
      {
        id: 'school',
        step: 4,
        term: '학교가 쓰고, 남으면 내보낸다',
        body:
          '만든 전기는 학교가 먼저 쓴다. 쓰는 곳에서 바로 만드니 멀리 보내며 잃는 전기가 없다. '
  ...
  production: {
    ...
    notes: [
      {
        id: 'shape',
        term: '곡선의 모양은 태양이 지나간 길과 같다',
        body:
          '봉우리가 솟은 시각이 태양이 가장 높이 뜬 때다. 옆 그림에서 태양이 오르내린 모양이 그대로 '
          + '곡선이 된다. 발전량을 정하는 것은 설비가 아니라 햇빛이다.',
      },
      {
        id: 'cloud',
        term: '움푹 팬 구간은 구름이 지나간 자리다',
        body: '태양이 잠깐 가려지면 그 시간만 발전량이 떨어졌다가, 구름이 지나가면 다시 올라온다.',
      },
    ],
  },
  benefit: {
    ...
    note: (scopeLabel, stats) =>
      `${scopeLabel}에서 금일 만든 ${formatNumber(stats.dayKwh)}kWh 를 다른 단위로 바꿔 본 값이다`,
    caption: '여기서 만든 만큼 화력발전이 줄어든다. 그루 수는 줄어든 탄소를 소나무가 흡수하는 양으로 바꾼 값이다.',
    itemIds: ['co2', 'tree', 'household', 'led'],
  },
  facts: [
    '태양전지는 온도가 높으면 오히려 효율이 떨어진다. 한여름보다 볕 좋은 봄가을에 발전량이 더 나온다.',
    '판에 먼지가 쌓이면 발전량이 줄어든다. 비가 한 번 내리면 그만큼 회복된다.',
    '직렬로 이은 판 하나에 그늘이 지면 그 줄 전체의 출력이 함께 떨어진다.',
