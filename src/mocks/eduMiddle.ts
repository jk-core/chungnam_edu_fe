...
export const MIDDLE_CONTENT: MiddleContent = {
  ...
  */
  ...
  headline: {
    mainLabel: '실시간 출력',
    mainNote: (stats) =>
      `설비용량 ${formatNumber(stats.capacityKw)}kW 로 낼 수 있는 최대치의 이만큼을 지금 내고 있다`,
    ...
    stages: [
      {
        ...
        body:
          ...
          + '데다 지나야 할 공기층까지 두꺼워, 판에 닿을 때는 이미 에너지가 많이 줄어 있다.',
      },
      {
        id: 'cell',
        step: 2,
    ...
    notes: [
      {
        ...
        body:
          '차트가 가장 높은 시각이 태양이 가장 높이 뜬 때다. 옆 그림에서 태양이 오르내린 모양이 그대로 '
          + '차트 모양이 된다. 발전량을 정하는 것은 설비 성능이 아니라 그 시각에 들어온 햇빛의 양이다.',
      },
      {
        id: 'cloud',
  ...
  facts: [
    ...
    '판 표면에 먼지가 쌓이면 발전량이 줄어든다. 비가 한 번 내리면 그만큼 회복된다.',
    '직렬로 이은 판 하나에만 그늘이 져도 그 줄 전체의 출력이 그 판에 맞춰 함께 떨어진다.',
